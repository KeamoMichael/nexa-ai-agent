...
export const TopBar: React.FC<TopBarProps> = ({ currentModel, setCurrentModel, credits, openCredits }) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 bg-[#F9F9F9]/90 backdrop-blur-sm">
      ...
      <button 
        onClick={openCredits}
        className="flex items-center gap-2 bg-gray-100 px-3 py-1.5 rounded-full text-xs font-medium hover:bg-gray-200 transition-colors"
      >
        <Sparkles size={12} className="text-gray-600" />
        <span className="text-gray-900">{credits}</span>
        <span className="text-blue-600 font-semibold ml-1">Upgrade</span>
      </button>
    </div>
  );
};